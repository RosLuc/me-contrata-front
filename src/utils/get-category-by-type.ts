...
export function getCategoryByType(type: FilterType) {
	if (type == FilterType.PROGRAMMING) return "mugs";
	if (type == FilterType.DESIGN) return "t-shirts";
	return "";
}
...
`;
	const sortSettngs = getFieldByPriority(priority);
	const categoryFilter = getCategoryByType(type);
	return `
      query {
        allProducts(sortField: "${sortSettngs.field}", 
        sortOrder: "${sortSettngs.order}", 
